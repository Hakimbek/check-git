...
const { CONTENT_TYPES } = require('../config/appConfig');
...
class PardotService {
    static sendTrialUserData(body, registrationFailed, requestData) {
        const pardotSettings = varsService_1.VarsService.get('pardotSettings');
        const { emailAddress, firstName, lastName, companyName, zipCode, phoneNo, acReferSource, subSegment: companySize, segment: companyType, language, } = body;
        ...
        const pardotQueryParams = Object.assign(Object.assign({ emailAddress,
            ...
            phoneNo,
            language,
            acReferSource }, this.getAggregatedUtmParams(body)), (registrationFailed && { registrationFailed }));
        let paramsFormData = '';
        Object.keys(pardotQueryParams).forEach(key => {
            if (pardotQueryParams[key]) {
                paramsFormData += `${key}=${pardotQueryParams[key]}&`;
            }
        });
